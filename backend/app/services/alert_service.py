...
def create_alert(db: Session, payload: AlertCreate, *, fanout: bool = True) -> Alert:
    ...
    if not data.get("link"):
        data["link"] = DEFAULT_LINKS.get(data.get("alert_type", ""), "/alerts")
    a = Alert(**data)
    db.add(a)
    db.flush()
    if fanout:
...
def mark_all_alerts_read(db: Session, tenant_id: int, user: User | None = None) -> int:
    items, _, _ = list_alerts(db, tenant_id, status="active", user=user, page=1, page_size=5000)
    count = 0
    for a in items:
        if not a.is_read:
            a.is_read = True
...
def sync_low_stock_alerts(db: Session, tenant_id: int) -> list[Alert]:
    ...
    existing = list(
        db.scalars(
            select(Alert).where(
                ...
                Alert.alert_type.in_(("low_stock", "out_of_stock", "reorder_level")),
                Alert.status == "active",
    ...
    db.commit()
    items, _, _ = list_alerts(db, tenant_id, alert_type="low_stock", status="active", page_size=500)
    out_items, _, _ = list_alerts(db, tenant_id, alert_type="out_of_stock", status="active", page_size=500)
