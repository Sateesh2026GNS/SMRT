...
def get_material_availability(db: Session, tenant_id: int) -> list[MaterialAvailabilityRead]:
    ...
    if not items:
        return [
            MaterialAvailabilityRead(product_name="Chair", material_status="available", available=True),
            MaterialAvailabilityRead(product_name="Table", material_status="shortage", available=False),
            MaterialAvailabilityRead(product_name="Steel Frame", material_status="available", available=True),
        ]
    return items
...
def get_shift_schedule(db: Session, tenant_id: int) -> list[ShiftScheduleItemRead]:
    ...
    machines = list(db.scalars(select(Machine).where(Machine.tenant_id == tenant_id)).all())
    items = []
    shift_names = ["Morning Shift", "Afternoon Shift", "Night Shift"]
    for i, label in enumerate(shift_names):
        ...
        items.append(
            ShiftScheduleItemRead(
                shift_name=shifts[i].name if i < len(shifts) else label,
                machine_name=m.name if m else f"Machine {i + 1}",
                operator_name="Ravi Kumar" if i == 0 else "Mahesh Patel" if i == 1 else "Suresh Reddy",
                product_name=product.name if product else ["Chair", "Table", "Steel Frame"][i % 3],
                quantity=float(wo.planned_quantity if wo else [1000, 700, 500][i % 3]),
                status="running" if i == 0 else "planned",
            )
        )
    return items
